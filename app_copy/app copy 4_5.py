...
with right_col:
    ...
    if start_analysis and image_file:
        ...
        st.markdown(f"**감정 상태:** :blue[{emotion_result}]")
        with st.expander("왜 이런 감정 상태가 나왔나요?"):
            st.write(f"AI는 표정 이미지에서 {emotion_result}에 해당하는 특징(눈썹의 위치, 입꼬리 방향 등)을 감지했기 때문에 이렇게 분류했어요.")

        st.markdown(f"**건강 상태:** :orange[{health_flag}]")
        with st.expander("왜 이런 건강 상태가 나왔나요?"):
            if health_flag == "양호":
                st.write("사용자가 선택한 활동 상태(예: 걷는 중, 앉아 있음)는 비교적 활력이 있는 상태로 판단됩니다.")
            else:
                st.write("정지 상태나 무기력함은 건강 위험 징후일 수 있어 '주의 필요'로 분류했어요.")
        ...
        with st.expander("이 행동은 왜 추천된 건가요?"):
            st.write(f"현재 감정 상태가 **{emotion_result}**로 분류되었기 때문에, 이에 적합한 행동으로 **{recommended_action}**이 추천되었어요.")
